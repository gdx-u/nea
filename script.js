...
function get_relevant_tiles(x, y, range, transform) {
    ...
    return Tile.tiles.filter(e => {return transform(e, x, y).reduce((c, v) => {return c + v * v}, 0) <= range * range});
}
...
class Tile {
    ...
    */
    constructor(x, y, information, append) {
        ...
        if (this.append) {
            tile_holder.appendChild(this.el);
...
class Player {
    ...
    async tick() {
        ...
        for (let tile of Tile.gates) {
            if (!tile.information.become_active) continue;
        ...
        if (!prev_tick) {
            ...
            //     // fps_el.innerText = `${fps_value}FPS`;
        ...
        this.indicator.style.left = (cx - this.iw / 2) + 'px';
        this.indicator.style.top = (cy - this.ih / 2) + 'px';
...
const tile_holder = document.getElementById("tiles");

function tile_tick() {
    // tile_holder.style.left = `${draw_x - player_x}px`;
    // tile_holder.style.top = `${draw_y - player_y}px`;
    tile_holder.style.transform = `translate(${draw_x - player_x}px, ${draw_y - player_y}px)`;
}
...
async function load_room(off_x, off_y, room_id, entrance, depth, from_id) {
    ...
    if (entrance && room_id !== 1) {
        ...
        let g = new Tile(gx + (gw < gh ? 4 * pxl : 0), gy + (gw > gh ? 4 * pxl : 0), {
            type: "wall",
            width: gw - (gw < gh ? 8 * pxl : 0),
            height: gh - (gw > gh ? 8 * pxl : 0),
            is_gate: true,
            colour: "#bbb",
            classes: ["gate", gw === door_width ? "horizontal" : "vertical", random_choice(gw === door_width ? ["left", "right"] : ["top", "bottom"])]
        });
    ...
    if (maze_gen_test) {
        ...
        tile_holder.appendChild(background_el);
    }
